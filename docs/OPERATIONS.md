...
- Export and download routes already fail closed on `.docx`, `.pdf`, and `.doc` with HTTP 422 `outbound_attribution_blocked`. Sponsor-CI also scans `.txt` and every other filename, so renaming a file cannot skip the gate. With the mode off, those other types still pass through and the Word/PDF gate stays as shipped.
