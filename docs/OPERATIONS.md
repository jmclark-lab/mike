...
- **Default first model:** `fugu-ultra-20260615` (Sakana Fugu Ultra). Fugu stays the lead seat for default / cheap / most legal-and-reg questions. It is priced well below Fable 5 at the same intelligence class (see https://sakana.ai/fugu/ : Fugu Ultra $5/1M input, $30/1M output, $0.50 cached) and is an orchestrator over a model pool.
- **Intent first-pick (deterministic, not an LLM router):** classify from the last user message / named tool. High-stakes paste-into-contract work (`draft_redlines`, `review_against_playbook`, `draft_contract`, a novel pathway that will be signed) starts on `claude-opus-4-8`. Everything else — including unsure — stays on Fugu Ultra. `convene_council` remains an explicit tool, never the default path.
- **Configured fallback pool (streaming chat):** `fugu-ultra-20260615` → `claude-fable-5` → `claude-opus-4-8` → `gpt-5.6-sol`. After the intent first pick, this pool is tried in order, health-reordered. Configurable via `LLM_MODEL` (pool primary) and `LLM_FALLBACK_MODEL` (comma-separated tail).
...
- **Legal council:** strict 4/4 quorum across Fable 5, Fugu Ultra, GPT-5.6 Sol Ultra (`gpt-5.6-sol` with `xhigh` reasoning), and the configured Gemini Pro seat. Each original model is retried up to `COUNCIL_MEMBER_MAX_ATTEMPTS` (default 3) without substitution. The Opus 4.8 judge is never invoked on partial quorum; an incomplete council fails explicitly with model-specific errors. Council seats are configured with `COUNCIL_ANTHROPIC_MODEL`, `COUNCIL_SAKANA_MODEL`, `COUNCIL_OPENAI_MODEL`, `COUNCIL_GEMINI_MODEL`, and `COUNCIL_GEMINI_LABEL`. Provider-specific output budgets can be tuned with `COUNCIL_ANTHROPIC_MAX_TOKENS`, `COUNCIL_SAKANA_MAX_TOKENS`, `COUNCIL_OPENAI_MAX_TOKENS`, `COUNCIL_GEMINI_MAX_TOKENS`, and `COUNCIL_JUDGE_MAX_TOKENS`; defaults are sized for reasoning-heavy contract reviews.
- **Search router:** `SEARCH_ROUTER_MODE` stays at its current default (`off` in repo; prod has been `shadow`). There is no in-repo A/B evidence that `live` is better, so this change does not flip it.
- **Gemini 3.5 Pro cutover:** as of 2026-07-15 Google has not published the exact Gemini 3.5 Pro API identifier. On release, verify the identifier with a direct API call in staging, set `COUNCIL_GEMINI_MODEL` and `COUNCIL_GEMINI_LABEL`, redeploy staging, run a real 4/4 council smoke test, then promote to production. Never guess an unreleased model id in production.
...
- **`GET /healthz`** — DB check + uptime + deployed `commit` + live routing/cooldown state (configured chain, first model by intent bucket, which provider keys are configured — booleans only); returns 503 if the DB is down. (There's also a trivial `GET /health` → `{ok:true}`.)
- **Per-call telemetry** — one JSON line per LLM call: `[llm.telemetry] {event:"llm_call", surface, ok, route_bucket, first_model, answered, fallback_depth, attempted[], empty, latency_ms, error_class, input_tokens?, output_tokens?}`. Token fields are included only when the provider already returned them — no invented dollar amounts. Grep Railway logs, or add a log drain to Axiom/Better Stack and alert when the `fallback_depth>0` share is high.
