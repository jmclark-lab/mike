...
create table if not exists public.kb_documents (
  ...
  source_ref text,
  source_tag text,
  tenant text not null default 'bioaccess' check (tenant in ('bioaccess', 'amavita')),
  source_url text,
  content_hash text,
  drive_file_id text,
  drive_version text,
...
create index if not exists kb_documents_owner_tenant_idx on public.kb_documents(owner_id, tenant);
...
create or replace function public.match_kb_chunks(
  query_embedding vector(1536), match_owner uuid, match_count int default 6, filter_doc_type text default null, filter_tenant text default null
)
returns table (
  chunk_id uuid, document_id uuid, title text, doc_type text,
  chunk_index int, content text, similarity float, tenant text
)
language sql stable
as $$
